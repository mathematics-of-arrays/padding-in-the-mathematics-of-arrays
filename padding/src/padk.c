...
void padk() {
  ...
  for (int part = 0;part<3;part++) {
    ...
    for (int i = 0; i<s0; i++) {
      for (int j = 0; j<s1; j++) {
        // first pad value
        upadded[offsetpadded+gamma_padded(i,j,0)]=start[offset+_gamma(i,j,s2-1)];
        // last pad value
        upadded[offsetpadded+gamma_padded(i,j,s2padded-1)]=start[offset+_gamma(i,j,0)];

        memcpy(upadded+offsetpadded+gamma_padded(i,j,1),start+offset+_gamma(i,j,s2-1),s2*sizeof(double));
      }
    }
  }
